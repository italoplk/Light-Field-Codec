...
TEST_F(TransformContextTest, dct_recovered_signal_within_error_margin) {
  ...
  EXPECT_NEAR(distance_percent<float>(signal, recovered_signal, FULL_LENGTH), 0,
              ERROR_REC_EPSILON)
...
TEST_F(TransformContextTest,
       ensure_dct_and_dst_generate_different_coefficients) {
  ...
  ASSERT_GE(distance_percent<float>(transformed_signal, transformed_signal_evil,
                                    FULL_LENGTH),
            0.01)
      << "The proportional distance between the DST and DCT "
