...
TransformContext<float> *create_transform(std::string transform,
                                          const Point4D &size,
                                          const Point4D &stride) {
  if (transform == "DCT")
    return new DiscreteCosineTransformContext4D<float>(size, stride);
  else if (transform == "DST")
    return new DiscreteSineTransformContext4D<float>(size, stride);
  throw std::invalid_argument("Unkown transform: " + transform);
}

template <class T> void printVector(const std::string &msg, T *vet, int size) {
...
int main(int argc, char **argv) {
  ...
  file_traceTransf << "channel" << sep << "pos_x" << sep << "pos_y" << sep
                   << "pos_u" << sep << "pos_v" << sep << "bl_x" << sep
                   << "bl_y" << sep << "bl_u" << sep << "bl_v" << sep;
  ...
  file_traceQuant << "channel" << sep << "pos_x" << sep << "pos_y" << sep
                  << "pos_u" << sep << "pos_v" << sep << "bl_x" << sep << "bl_y"
                  << sep << "bl_u" << sep << "bl_v" << sep;
  ...
  file_traceLRE << "channel" << sep << "pos_x" << sep << "pos_y" << sep
                << "pos_u" << sep << "pos_v" << sep << "bl_x" << sep << "bl_y"
                << sep << "bl_u" << sep << "bl_v" << sep << "level" << sep
                << "run" << std::endl;
#endif
  ...
  for (it_pos.v = 0; it_pos.v < dimLF.v; it_pos.v += dimBlock.v) { // angular
    for (it_pos.u = 0; it_pos.u < dimLF.u; it_pos.u += dimBlock.u) {
      ...
      for (it_pos.y = 0; it_pos.y < dimLF.y;
           it_pos.y += dimBlock.y) { // spatial
        for (it_pos.x = 0; it_pos.x < dimLF.x; it_pos.x += dimBlock.x) {
          ...
          std::cout << "Pos: " << it_pos << "\tBlock Size: " << dimBlock
                    << std::endl;
          for (int i = 0; i < encoderParameters.dim_block.getNSamples(); ++i) {
...
predictor.predict(orig4D, encoderParameters.dim_block, pf4D);
            ...
            file_traceTransf << it_channel << sep <<
                ...
                it_pos.x << sep << it_pos.y << sep << it_pos.u << sep
                             << it_pos.v << sep <<
...
#if DPCM_DC
            qf4D[0] -=
                (float)dpcmDc[it_channel].get_reference(it_pos.x, it_pos.y);
#endif
            ...
            for (int i = 0; i < encoderParameters.dim_block.getNSamples();
                 ++i) {
              temp_lre[i] = (int)std::trunc(qf4D[i]);
            }

            auto lre_result = lre.encodeCZI(
                temp_lre, 0, encoderParameters.dim_block.getNSamples());

            bits_per_4D_Block = encoder.write4DBlock(
                temp_lre, encoderParameters.dim_block.getNSamples(),
                lre_result);

#if TRACE_QUANT
            file_traceQuant << it_channel << sep << it_pos.x << sep << it_pos.y
                            << sep << it_pos.u << sep << it_pos.v << sep <<
            ...
            for (auto it_lre : lre_result) {
              file_traceLRE << it_channel << sep << it_pos.x << sep << it_pos.y
                            << sep << it_pos.u << sep << it_pos.v << sep <<
            ...
            std::copy(tf4D, tf4D + encoderParameters.dim_block.getNSamples(),
                      qi4D);
#endif
