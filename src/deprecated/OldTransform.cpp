...
void Transform::dct_4d(const float *input, float *output, const Point4D &size,
  ...
  const uint size_x = size.x, size_y = size.y, size_u = size.u, size_v = size.v;

  // TODO: Refactor these variables to something more meaninful
  const uint offset1 = origSize.x, offset2 = offset1 * origSize.y,
             offset3 = offset2 * origSize.u, offset4 = offset3 * origSize.v;
  ...
  for (int v = 0; v < size_v; ++v) {
    for (int u = 0; u < size_u; ++u) {
      for (int x = 0; x < size_x; ++x) {
        dct_1D(p_input, p_output, this->coeff_dct1D[1], offset1,
               size_y); // Y AXIS
  ...
  for (int v = 0; v < size_v; ++v) {
    for (int y = 0; y < size_y; ++y) {
      for (int x = 0; x < size_x; ++x) {
        dct_1D(p_input, p_output, this->coeff_dct1D[2], offset2,
               size_u); // U AXIS
  ...
  for (int u = 0; u < size_u; ++u) {
    for (int y = 0; y < size_y; ++y) {
      for (int x = 0; x < size_x; ++x) {
        dct_1D(p_input, p_output, this->coeff_dct1D[3], offset3,
               size_v); // V AXIS
 ...
 * }
 */
void Transform::dct_1D(const float *in, float *out, float *coeff,
 ...
 * }
 */
void Transform::idct_1D(const float *in, float *out, float *coeff,
                        const uint offset, const uint size) {
  int N = size;
...
void Transform::idct_4d(const float *input, float *output, const Point4D &size,
                        const Point4D &origSize) {
  if (this->dim_block != size) {
    this->delete_coeff_dct();
    this->alocate_coeff_dct(size);
  ...
  for (int u = 0; u < size_u; ++u) {
    for (int y = 0; y < size_y; ++y) {
      for (int x = 0; x < size_x; ++x) {
        idct_1D(p_input, p_output, this->coeff_dct1D[3], offset3,
                size_v); // V AXIS
  ...
  for (int v = 0; v < size_v; ++v) {
    for (int y = 0; y < size_y; ++y) {
      for (int x = 0; x < size_x; ++x) {
        idct_1D(p_input, p_output, this->coeff_dct1D[2], offset2,
                size_u); // U AXIS
  ...
  for (int v = 0; v < size_v; ++v) {
    for (int u = 0; u < size_u; ++u) {
      for (int x = 0; x < size_x; ++x) {
        idct_1D(p_input, p_output, this->coeff_dct1D[1], offset1,
                size_y); // Y AXIS
...
void Transform::delete_coeff_dct() {
  ...
  if (dim_block.y != dim_block.x)
    delete[] this->coeff_dct1D[1];
  if (dim_block.u != dim_block.y && dim_block.u != dim_block.x)
    delete[] this->coeff_dct1D[2];
  if (dim_block.v != dim_block.y && dim_block.v != dim_block.x &&
      dim_block.v != dim_block.u)
...
void Transform::alocate_coeff_dct(const Point4D &dimBlock) {
  ...
  this->coeff_dct1D[1] = (this->dim_block.y == this->dim_block.x)
                             ? this->coeff_dct1D[0]
                             : generate_dct_coeff(this->dim_block.y);

  this->coeff_dct1D[2] = (this->dim_block.u == this->dim_block.x)
                             ? this->coeff_dct1D[0]
                             : (this->dim_block.u == this->dim_block.y)
                                   ? this->coeff_dct1D[1]
                                   : generate_dct_coeff(this->dim_block.u);

  this->coeff_dct1D[3] =
      (this->dim_block.v == this->dim_block.x)
          ? this->coeff_dct1D[0]
          : (this->dim_block.v == this->dim_block.y)
                ? this->coeff_dct1D[1]
                : (this->dim_block.v == this->dim_block.u)
                      ? this->coeff_dct1D[2]
                      : generate_dct_coeff(this->dim_block.v);
}
}; // namespace old